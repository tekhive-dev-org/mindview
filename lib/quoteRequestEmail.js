...
export function createQuoteRequestEmail({ data, requestId, submittedAt }) {
	...
	const subject = `New Mindview Automation quote request: ${data.companyName} · ${requestId}`;
...
<body style="margin: 0; padding: 0; background: #eef2ff; font-family: Arial, Helvetica, sans-serif; color: #0f172a;">
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" class="outer-pad" style="background: #eef2ff; padding: 32px 16px;">
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" class="card" style="max-width: 640px; background: #ffffff; border-radius: 24px; overflow: hidden; box-shadow: 0 20px 56px rgba(15,23,42,0.13);">
          ...
          <tr>
            <td class="hdr" style="padding: 36px 36px 32px; background: linear-gradient(135deg, #0f172a 0%, #004f9c 55%, #0ea5e9 100%);">
              <div style="display: inline-block; padding: 7px 14px; border-radius: 999px; background: rgba(255,255,255,0.15); font-size: 11px; line-height: 16px; font-weight: 700; letter-spacing: 0.09em; text-transform: uppercase; color: #ffffff; margin-bottom: 18px;">Mindview Automation Quote Desk</div>
              ...
              <p class="hdr-p" style="margin: 0; font-size: 15px; line-height: 24px; color: rgba(255,255,255,0.88); max-width: 480px;">A new commercial inquiry has been submitted through the Mindview Automation contact form.</p>
            </td>
          </tr>
          ...
          <tr>
            <td class="note-wrap" style="padding: 0 28px 32px;">
              <div class="note-inner" style="padding: 18px 22px; border-radius: 16px; background: #f8fafc; border: 1px solid #e2e8f0; font-size: 13px; line-height: 22px; color: #475569;">
                This email was generated from the Mindview Automation website. Replying routes back to <strong style="color: #0f172a;">${safeName}</strong> at <a href="mailto:${safeEmail}" style="color: #004f9c; text-decoration: none;">${safeEmail}</a>.
              </div>
            </td>
          </tr>
	...
	const text = [
		`New Mindview Automation quote request`,
		`Reference: ${requestId}`,
